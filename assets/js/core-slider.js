...
function initCoreSlider() {
    ...
    function createSlideElement(getItem, index) {
        ...
        if ( coreSlideSettings.InfoRottenTomatoes && getItem.CriticRating !== undefined && getItem.CriticRating !== null && getItem.CriticRating !== '' ) {
            var criticRatingValue = parseFloat(getItem.CriticRating);
            if ( !isNaN(criticRatingValue) && criticRatingValue > 0 ) {
                var rottenTomatoesIcon = criticRatingValue >= 60 ? icons.fresh_tomato : icons.rotten_tomato;

                var createSlideInfoRottenTomatoes = document.createElement('div');
                createSlideInfoRottenTomatoes.className = 'core-slide-info-rotten-tomatoes';
                createSlideInfoRottenTomatoes.innerHTML = '<p><span>' + rottenTomatoesIcon + '</span> ' + Math.round(criticRatingValue) + '%</p>';
                createSlideInfo.appendChild(createSlideInfoRottenTomatoes);
            }
        }
